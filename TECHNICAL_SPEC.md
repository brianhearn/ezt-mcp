...
- Full queued request payloads are stored in `transient.job_payloads`; `request_summary` may reference `payload_handle`/payload size metadata but must not embed full customer payloads after migration 003 is available. Temporary dev/staging compatibility mode may fall back to legacy `request_summary.request_payload` only when `transient.job_payloads` / migration-003 columns are absent.
...
The deployed implementation runs a startup `JobWorker` in `ezt_mcp/workers.py`. On submission, `direct_build` and `create_territory_from_parts` persist a queued job and return immediately. When migration 003 is present, full queued request payloads are stored in `transient.job_payloads` and referenced from `transient.jobs.payload_handle` / `request_summary.payload_handle`; `request_summary` remains summary metadata only. Until migration 003 lands, temporary compatibility mode stores the request payload in `request_summary.request_payload` so the branch remains deployable for dev/staging testing. The worker claims eligible rows from `transient.jobs`, dispatches by `tool_name`, hydrates the payload from either storage path, runs the Direct Build worker pipeline, updates job progress/results, and publishes best-effort Map Component progress events when the queued request includes `map_session_id`.

Claims set/extend a lease. If migration 003 is present and a worker dies after a job has transitioned to `running`, a later worker may reclaim the job once `lease_expires_at` has passed and `next_attempt_at` has arrived. Reclaimed jobs keep the same `job_id`, increment `attempt_count`, use configurable backoff, and fail with `JOB_ATTEMPTS_EXHAUSTED` once `max_attempts` is reached. In temporary pre-migration compatibility mode, claim logic only claims queued jobs and does not reclaim stale running rows because the required attempt/backoff columns are absent. Direct Build/create-territory execution must remain idempotent with respect to the transient TS/result model.
...
The upper-left panel is a template-driven context panel, not arbitrary agent-generated chrome. Agents may pass title/subtitle/summary/layer-legend/debug hints through presentation metadata, but the MC owns layout and product styling per `DESIGN.md`. The debug panel is controlled by `debug_panel` in the resolved presentation payload and should default off outside QA contexts. Customer-visible viewer strings are resolved from `presentation.chrome_labels` with built-in defaults; this keeps labels such as the active-alignment selector internationalizable and avoids leaking internal acronyms like “TAL” into product chrome.

The Layer-Legend view model should be resolved server-side from TS presentation metadata and session state rather than inferred entirely in static viewer JavaScript. It should normalize TALs, dimmed/reference TALs, point location layers, part overlays, and future overlays into a common list of layer rows with optional child classification rows. A row may include: stable `layer_id`, human label, layer kind, visibility state, disabled/out-of-scale state, swatch/symbol description, source TS layer reference, optional min/max zoom, optional class rows, and simple declarative filter state. Browser-side toggles should update map-session presentation state only; they must not mutate TS data or analysis semantics.

Point location layer class rows should be compatible with Designer-style classification concepts but intentionally smaller. Benton's `mapsjs/EasyTerritoryAI` app is the concrete reference: `MapLayersPanel` combines layer toggles with inline legends; `LayerClassificationPanel` supports color classification, size classification, filters, territory filter, min zoom, opacity, and clustering; `FiltersPanel` models operators `eq`, `neq`, `in`, `nin`, `lt`, `lte`, `gt`, `gte`, and `between`; `business-data-layer.ts` stores the resolved config. MC v1 should adopt the normalized view-model ideas and point-layer config shape where useful, but not copy the full editing panel. The MC should render categorical values, manual/numeric ranges, equal interval, quantile, visible/hidden class state, optional filter label, and optional precomputed counts/statistics. Complex filter-builder expressions, clustering, hotspots, and symbology editing stay out of the v1 implementation unless future specs explicitly add them.
