...
- maintain MC visual smoke coverage for every deploy that touches job, map-session, progress, cancellation, CSS, render-payload, layer-legend, point-layer rendering, classification/filter handling, or part-layer overlay paths; use `scripts/create_point_range_demo.py` for a reusable point-layer range-class demo when browser/user visual feedback is needed;
- run `scripts/smoke_direct_build.py --base-url https://expertpack.ai/mcp` for deploys touching Direct Build, queued jobs, PostGIS geometry fetch/dissolve, TAL metadata, Map Component render-payload creation, or part-layer overlay metadata; for large Excel/CSV alignment regressions, use `scripts/build_direct_from_excel.py` to generate assignment fixtures and remember that public nginx currently rejects ~2.5MB Direct Build JSON bodies (`413`), so full-file runs need either local service submission, staged handles, or an increased body-size limit;
...
- PowerBI demo ZIP alignment fixture (`tests/PowerBI_Demo_V5.xlsx`, `ZIP2Terr` tab) is now a useful large Direct Build regression source. The sheet has 41,076 unique ZIPs, 16 conflicting duplicate assignments, and 7,589 ZIPs missing from current `geo.us_postal` polygon coverage. A first-wins, found-only live run completed 33,487 assignments into an 88-territory TAL (80 leaves + 8 rollups). Treat missing ZIP geometry and large-request submission as explicit follow-up work, not Direct Build correctness failures.
